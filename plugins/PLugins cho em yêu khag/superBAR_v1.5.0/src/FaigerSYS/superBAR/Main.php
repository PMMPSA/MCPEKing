		...
		if($cmd->getName() == 'sbar') {
			if (count($args) == 0) {
				$sender->sendMessage(
					...
					$this->prefix . 'Commands list: ' . CLR::DARK_GREEN . '/sbar help'
				);
			} elseif ($args[0] == 'help') {
				...
				$sender->sendMessage($this->prefix . CLR::RED . 'Wrong command!' . CLR::DARK_GREEN . ' /sbar help ' . CLR::RED . 'for a list of commands.');
			}
		}
	}
